...
# Initialize HuggingFace API with token from environment (if available)
hf_token = os.getenv("HF_TOKEN")
hf_api = HfApi(token=hf_token) if hf_token else None

# Initialize FastMCP and assign to mcp
mcp = FastMCP()
...
def add_new_tag(repo_id: str, new_tag: str) -> str:
    ...
    try:
        ...
        try:
            ...
            print(f"❌ Error loading model card: {str(e)}")
            error_result = {"status": "error", "repo_id": repo_id, "error": f"Failed to load model card: {str(e)}"}
            json_str = json.dumps(error_result)
